...
## Из таска 10 — поиск и юридические страницы (принят 20.09.2026, коммиты `963f5ab`, `843abe1`)

`apps/web/src/app/sitemap.ts` строит карту из `GET /catalog/products` через `fetchCatalog`
постранично; `robots.ts` берёт адрес сайта и путь карты оттуда же. Адрес сайта — `metadataBase`
в `apps/web/src/app/layout.tsx`, записан один раз. Реестр юридических страниц —
