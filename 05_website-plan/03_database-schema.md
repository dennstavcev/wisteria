...
| is_active | boolean | ~~скрыт/показан на сайте (архивные товары МойСклад не показываем)~~ — **уточнено 04.09.2026:** признак «не в архиве» в МойСклад. Видимостью страницы на витрине он не управляет (§9): опубликованная карточка архивной позиции остаётся на сайте, у неё лишь пропадает возможность купить |
...
| email | text, nullable |
| name | text |
| role | enum(guest, retail_client, wholesale_client, **florist**, manager, admin) | `florist` добавлена 03.09.2026: сотрудник, который наполняет карточки и фото с телефона в зале, но не видит ни денег, ни клиентов — без отдельной роли этот доступ пришлось бы выдавать как `manager` (`18_admin-catalog-content.md` §12, `06_admin-dashboard.md` §8). Для `manager`/`admin` значение берётся только из `staff_allowlist` (ниже), а не из запроса |
| totp_secret_enc | text, nullable | секрет TOTP-приложения сотрудника (рекомендован Яндекс Ключ, `14_security-and-process-decisions.md` §2), зашифрованный ключом приложения из переменной окружения `TOTP_ENCRYPTION_KEY`. **В документах проекта приводится имя поля и имя переменной — значение секрета не приводится никогда** |
| totp_confirmed_at | timestamptz, nullable | момент, когда привязка подтверждена первым верным кодом. Пусто — второй фактор не привязан, и вход в управленку не открывается; это требование привязать фактор, а не разрешение войти без него |
| totp_last_step | bigint, nullable | номер последнего принятого 30-секундного шага TOTP — тот же код повторно не принимается (перехваченный код бесполезен уже на второй попытке) |
...
### 2.1 Второй фактор и allowlist служебных ролей

Подраздел добавлен 04.09.2026. Источник требования — `14_security-and-process-decisions.md` §2 (решение 02.09.2026) и его же §5в как условие приёмки кода. До этой даты в схеме не было ни полей TOTP, ни признака allowlist, из-за чего решение было нереализуемо миграцией Этапа 1 — расхождение №13 в `17_readiness-audit.md` §5.

**staff_allowlist** — единственный источник ролей `manager` и `admin`
| Поле | Тип | Комментарий |
|---|---|---|
| id | uuid (PK) | |
| provider | enum(telegram, vk) | канал, которым подтверждается личность при входе |
| external_id | bigint | `telegram_id` / `vk_id` того, кому открывается доступ; строка заводится **вручную**, приложение её не создаёт |
| role | enum(manager, admin) | роль, присваиваемая при успешном входе |
| note | text | кто это и на каком основании доступ выдан |
| created_at | timestamptz | |
| created_by | uuid, nullable (FK → users) | null — первая запись, заведённая при развёртывании |
| revoked_at | timestamptz, nullable | мягкий отзыв доступа; строка не удаляется (общее правило проекта — историю не стираем) |

Индекс: `(provider, external_id)` unique среди строк с `revoked_at is null`.

**Что означает allowlist.** Роль не приходит в запросе и не назначается кодом приложения на лету (`14_...` §5в). При каждом входе backend ищет пару `(provider, external_id)` среди неотозванных строк: строка есть — роль пользователя приводится к её значению; строки нет — роль остаётся клиентской, что бы клиент ни прислал. Отзыв доступа делается через `revoked_at`, а не правкой `users.role`: иначе следующий же вход вернёт роль обратно.

**Проверка второго фактора — на сервере при каждом входе `manager`/`admin`**, а не только при регистрации фактора (`14_...` §5в). Пустой `totp_confirmed_at` у пользователя со строкой в allowlist означает обязательную привязку до первого входа в управленку.

**Сетевое ограничение доступа к `admin.<домен>`** (IP-allowlist/VPN, `14_...` §2) схемой БД не выражается — это конфигурация Caddy и сети, отдельный слой поверх проверки роли; в таблицах его искать не нужно.

**Почему allowlist отдельной таблицей, а не колонкой в `users`**: строка allowlist должна существовать до того, как человек впервые вошёл и запись в `users` вообще появилась. Колонка заставила бы сначала создать пользователя обычным флоу и потом повысить ему роль — то есть пройти ровно тем путём присвоения роли, который `14_...` §2 запрещает. **Почему секрет TOTP в БД, а не в конфиге**: секрет у каждого сотрудника свой и заводится в момент привязки; в конфиге каждый новый человек стоил бы передеплоя.

**Роль `florist` в `staff_allowlist.role` намеренно не включена.** Она появилась 03.09.2026, на следующий день после решения `14_...` §2, и в нём не названа — дописывать её от себя значило бы придумать требование по безопасности за заказчика: `[ВТОРОЙ ФАКТОР И ALLOWLIST ДЛЯ `florist` — подтвердит заказчик]`. Расширить enum дешевле, чем отзывать уже выданный доступ.
...
| видимость на витрине | ~~`product_site.is_published = true` **и** `products.is_active = true` **и** `product_site.archived_at is null`~~ — **исправлено 04.09.2026:** `product_site.is_published = true` **и** `product_site.archived_at is null`; `products.is_active` в правило видимости не входит (разбор ниже) |
...
left join products p on p.moysklad_id = ps.moysklad_id       -- позиции может уже не быть
where ps.is_published and ps.archived_at is null;
```

**Направление соединения изменено 04.09.2026** (было ~~`from products p left join product_site ps`~~). Ведущая таблица — `product_site`: карточка переживает позицию, а при ведущей `products` исчезнувшая позиция уносила бы страницу из представления вместе с собой, что прямо запрещено ADR `docs/adr/0001-kartochka-sajta-otdelno-ot-zerkala-moysklad.md`. Обратная сторона названа честно: позиция без строки в `product_site` на витрину не попадает — публикация остаётся отдельным осознанным действием (`18_admin-catalog-content.md` §1.2, строка `is_published`).

### 9.1 Видимость страницы и возможность купить — разные условия

Зафиксировано 04.09.2026. Основание — ADR `docs/adr/0001-kartochka-sajta-otdelno-ot-zerkala-moysklad.md`: жизненный цикл карточки сайта не привязан к жизненному циклу позиции МойСклад, позиция исчезает — карточка остаётся. Та же формулировка слово в слово — `18_admin-catalog-content.md` §1.2; до этой даты два файла давали на один вопрос два ответа.

- **Страница видна на витрине**, если `product_site.is_published = true` **и** `product_site.archived_at is null`. Больше в это условие не входит ничего.
- **Товар можно купить**, если строка в `products` есть, `products.is_active = true` и остаток по `stock_levels` больше нуля.

Второе условие ложно — страница остаётся: URL, текст, фото, метатеги и накопленный поисковый вес живы, `availability` переключается в `OutOfStock`, на месте кнопки «в корзину» — подбор похожего товара (`18_admin-catalog-content.md` §7, `15_seo-promotion.md` §2.5).

**Почему `products.is_active` убран из правила видимости**: в конъюнкции архивация позиции в МойСклад молча гасила бы опубликованную страницу вместе со всем, что она набрала в поиске, — а у цветочного магазина позиция живёт дни, тогда как страница должна жить годами. Гасить страницу вправе только человек, полем `archived_at`. Это не ослабление контроля наличия: наличие по-прежнему целиком за МойСклад (§1, `18_...` §1.3), просто наличие управляет кнопкой, а не существованием страницы.
